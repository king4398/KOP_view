...
function updateLegend(){
  ...
  if(currentVar==="temperature"){
    ...
    els.legendBox.innerHTML=`<div style="font-weight:bold; margin-bottom:6px;">Elevation [m]</div><div style="width:220px; height:16px; background:linear-gradient(to right, #0d266b, #1f5cad, #6baed6, #f6f6ef, #f4a582, #d94830, #800026); border:1px solid #666;"></div><div style="display:flex; justify-content:space-between; font-size:12px; margin-top:4px;"><span>${v.vmin}</span><span>${((v.vmin+v.vmax)/2).toFixed(2)}</span><span>${v.vmax}</span></div>`;
  } else {
...
// KOP_FINAL_TUNE_MESH018_FLOW0063_GRAYPARTICLE_SMOOTHRDBU
