...
function updateScreenCoordinates() {
    if (!meshLonLat || !screenCoords || !map || !gl) return;

    const n = meshLonLat.length / 2;

    for (let i = 0; i < n; i++) {
        const lon = meshLonLat[i * 2];
        const lat = meshLonLat[i * 2 + 1];
...
function renderScalar() {
    ...
    if (positionsDirty) updateScreenCoordinates();
