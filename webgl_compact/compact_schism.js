...
const CONFIG = {
  ...
  nodesUrl: DATA_ROOT + "mesh_nodes.bin.gz",
  elemsUrl: DATA_ROOT + "mesh_elems.bin.gz",
  edgesUrl: DATA_ROOT + "mesh_edges.bin.gz",
  lookupMetaUrl: DATA_ROOT + "lookup_meta.json",
  lookupOffsetsUrl: DATA_ROOT + "lookup_offsets.bin.gz",
  lookupTrianglesUrl: DATA_ROOT + "lookup_triangles.bin.gz",
  flowScale: 0.005,
...
function scalarFrameUrl(variable, frameIndex) {
    if (variable === "temperature") return DATA_ROOT + `temp_q16/frame_${pad4(frameIndex)}.bin.gz`;
    if (variable === "ssh") return DATA_ROOT + `ssh_q16/frame_${pad4(frameIndex)}.bin.gz`;
    return null;
}

function currentUUrl(frameIndex) {
    return DATA_ROOT + `current_u_q16/frame_${pad4(frameIndex)}.bin.gz`;
}

function currentVUrl(frameIndex) {
    return DATA_ROOT + `current_v_q16/frame_${pad4(frameIndex)}.bin.gz`;
}
...
async function fetchArrayBuffer(url) {
    ...
    if (url.endsWith(".gz")) {
        if (typeof pako === "undefined") {
            throw new Error("pako is required to inflate gzip data");
        }
...
function q16Info(variable, frameIndex) {
    if (!q16Meta || !q16Meta.frames) {
        throw new Error("q16 metadata is not loaded");
    }

    const f = q16Meta.frames[String(frameIndex)] || q16Meta.frames[frameIndex];

    if (!f || !f[variable]) {
        throw new Error(`No q16 metadata for ${variable} frame ${frameIndex}`);
    }

    return f[variable];
}

async function fetchQ16Frame(url, variable, frameIndex, expectedLength = null) {
    const buf = await fetchArrayBuffer(url);
    const q = new Int16Array(buf);

    if (expectedLength !== null && q.length !== expectedLength) {
        throw new Error(`${url}: Int16 length ${q.length} != expected ${expectedLength}`);
    }

    const info = q16Info(variable, frameIndex);
    const out = new Float32Array(q.length);

    const vmin = info.min;
    const vmax = info.max;
    const scale = (vmax - vmin) / 65534.0;

    for (let i = 0; i < q.length; i++) {
        const qi = q[i];

        if (qi === -32768) {
            out[i] = meta.invalid_value;
        } else {
            out[i] = vmin + (qi + 32767) * scale;
        }
    }
...
async function loadScalarFrame(v,i){ const url=scalarFrameUrl(v,i); if(!url) return null; const key=`${v}:${i}`; if(scalarCache.has(key)) return scalarCache.get(key); if(scalarLoading.has(key)) return await scalarLoading.get(key); const promise=fetchQ16Frame(url, variable, frameIndex, meta.node_count).then(arr=>{ scalarCache.set(key,arr); scalarLoading.delete(key); for(const k of Array.from(scalarCache.keys())){ const [vv,ff]=k.split(":"); if(vv!==v || Math.abs(parseInt(ff)-i)>2) scalarCache.delete(k); } return arr; }); scalarLoading.set(key,promise); return await promise; }
...
function preloadCurrentFrame(i){ const next=Math.min(frameCount()-1,i+1), key=String(next); if(currentCache.has(key)) return; Promise.all([fetchQ16Frame(currentUUrl(next), "current_u", next, meta.node_count),fetchQ16Frame(currentVUrl(next), "current_v", next, meta.node_count)]).then(([u,v])=>currentCache.set(key,{u,v})).catch(()=>{}); }
...
async function boot(){ setStatus("Loading metadata..."); meta=await fetchJson(CONFIG.metaUrl); lookupMeta=await fetchJson(CONFIG.lookupMetaUrl);
    q16Meta = await fetchJson(DATA_ROOT + "q16_meta.json"); els.frameSlider.max=frameCount()-1; setStatus("Loading mesh and lookup..."); [nodesLonLat,elems,meshEdges,lookupOffsets,lookupTriangles]=await Promise.all([fetchFloat32(CONFIG.nodesUrl,meta.node_count*2),fetchUint32(CONFIG.elemsUrl,meta.index_count),fetchUint32(CONFIG.edgesUrl),fetchUint32(CONFIG.lookupOffsetsUrl),fetchUint32(CONFIG.lookupTrianglesUrl)]); initMap(); map.on("load",async()=>{ setStatus("Preparing WebGL layer..."); buildMercatorNodes(); initParticleCanvas(); map.addLayer(makeSchismLayer()); setupEvents(); updateCurrentOverlayAvailability(); updateLegend(); await setFrame(0); setStatus(`Ready: ${meta.node_count.toLocaleString()} nodes, ${meta.triangle_count.toLocaleString()} triangles`); }); }
